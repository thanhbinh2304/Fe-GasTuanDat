...
export default function ImportReceiptDetailModal({
  order,
  onClose,
  onSave,
  onDelete,
}: ImportReceiptDetailModalProps) {
  ...
  const handleDeleteClick = () => {
    showConfirm(
      'Xóa phiếu nhập hàng',
      'Bạn có chắc chắn muốn xóa phiếu nhập hàng này không ? Hành động này không thể hoàn tác.',
      async () => {
        try {
      ...
      <div className={styles.backdrop}>
        <div className={styles.container}>
          <div className={styles.header}>
            <h2 className={styles.headerTitle}>Chi tiết phiếu nhập hàng</h2>
          ...
          <div className={styles.body} style={{ padding: '40px 20px', textAlign: 'center', fontSize: '16px', lineHeight: '1.5' }}>
            Bạn muốn thao tác gì với thông tin phiếu nhập hàng <strong>{order.purchaseCode}</strong>?
          </div>
