...
import { X, Trash2, Save, KeyRound, Eye, EyeOff } from 'lucide-react';
...
export default function AccountDetailModal({ account, accounts, onClose, onSave, onDelete }: AccountDetailModalProps) {
  ...
  const handleSave = () => {
    if (!formData.username?.trim() || !formData.roleId || !formData.employeeId) {
      showToast('Thao tác không thành công', 'error');
      return;
    }

    const existingAcc = accounts.find(a => a.employeeId === formData.employeeId && a.id !== account.id);
    if (existingAcc) {
  ...
  const handleDeleteClick = () => {
    showConfirm(
      'Xóa tài khoản',
      `Bạn có chắc chắn muốn xóa tài khoản ${account.code} không?`,
      () => {
        onDelete(account.id);
        showToast('Thao tác thành công', 'success');
      }
    ...
    <div className={styles.backdrop}>
      <div className={styles.container} style={{ maxWidth: '600px' }}>
        <div className={styles.header}>
          <h2 className={styles.headerTitle}>Cập nhật tài khoản</h2>
