...
import { reportService, GasBookReportItem } from '@/services/reportService';

const formatCurrency = (amount: number) => new Intl.NumberFormat('vi-VN').format(amount);

export default function GasBookReportPage() {
  const [startDate, setStartDate] = useState(new Date(new Date().setMonth(new Date().getMonth() - 1)).toISOString().split('T')[0]);
  const [endDate, setEndDate] = useState(new Date().toISOString().split('T')[0]);
  const [data, setData] = useState<GasBookReportItem[]>([]);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const result = await reportService.getGasBookSummaryReport(startDate, endDate);
      setData(result);
    } catch (error) {
      console.error('Lỗi khi lấy dữ liệu báo cáo sổ ga:', error);
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className={styles.stockTableWrapper}>
              <table className={styles.table}>
                <thead className={styles.thead}>
                  <tr>
                    <th style={{ padding: '12px 16px', fontWeight: 600, textAlign: 'left', borderBottom: '1px solid var(--border-color)' }}>Mã sổ gas</th>
                    <th style={{ padding: '12px 16px', fontWeight: 600, textAlign: 'left', borderBottom: '1px solid var(--border-color)' }}>Tên khách hàng</th>
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={7} style={{ textAlign: 'center', padding: '20px' }}>Không có dữ liệu sổ gas</td>
                    </tr>
                  ) : (
                    data.map((d, i) => (
                      <tr key={i} className={styles.tr}>
                        <td style={{ padding: '12px 16px', color: 'var(--primary-color)' }}>{d.gasBookCode.substring(0, 8).toUpperCase()}</td>
                        <td style={{ padding: '12px 16px', textAlign: 'left' }}>{d.customerName}</td>
      ...
      <div className={styles.printArea} style={{ display: 'none' }}>
        <div className={styles.printHeader}>
          ...
          <div className={styles.printHeaderRight}>
            <div className={styles.printTitle}>BÁO CÁO SỔ GAS</div>
        ...
        <table className={styles.printTable}>
          ...
          <tbody>
            {data.map((d, i) => (
              <tr key={i}>
                <td>{d.gasBookCode.substring(0, 8).toUpperCase()}</td>
                <td style={{ textAlign: 'left' }}>{d.customerName}</td>
