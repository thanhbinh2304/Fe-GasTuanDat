...
import { reportService, CustomerReportItem } from '@/services/reportService';
...
export default function CustomerReportPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError('');
      try {
        const result = await reportService.getCustomerSummaryReport(startDate, endDate);
        setData(result || []);
      } catch (err: any) {
        console.error('Lỗi khi tải dữ liệu báo cáo khách hàng:', err);
        setError('Không thể tải dữ liệu báo cáo khách hàng.');
      } finally {
        setLoading(false);
      }
    };
            ...
            <div className={styles.stockTableWrapper}>
              <table className={styles.table}>
                <thead className={styles.thead}>
                  <tr>
                    <th style={{ padding: '12px 16px', fontWeight: 600, textAlign: 'left', borderBottom: '1px solid var(--border-color)' }}>Mã khách hàng</th>
                    ...
                    data.map((d, i) => {
                      ...
                      return (
                        <tr key={i} className={styles.tr}>
                          <td style={{ padding: '12px 16px', color: 'var(--primary-color)' }}>{d.customerCode}</td>
      ...
      <div className={styles.printArea} style={{ display: 'none' }}>
        <div className={styles.printHeader}>
          ...
          <div className={styles.printHeaderRight}>
            <div className={styles.printTitle}>BÁO CÁO KHÁCH HÀNG</div>
