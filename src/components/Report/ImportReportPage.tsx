...
import { reportService, PurchaseOrderReportItem } from '@/services/reportService';
...
export default function ImportReportPage() {
  ...
  const [data, setData] = useState<PurchaseOrderReportItem[]>([]);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const result = await reportService.getImportSummaryReport(startDate, endDate);
      setData(result);
    } catch (error) {
      console.error('Lỗi khi lấy dữ liệu báo cáo nhập hàng:', error);
    } finally {
      setLoading(false);
    }
  };
              ...
              <table className={styles.table}>
                <thead className={styles.thead}>
                  <tr>
                    ...
                    <th style={{ padding: '12px 16px', fontWeight: 600, textAlign: 'center', borderBottom: '1px solid var(--border-color)' }}>Tổng số phiếu nhập hàng</th>
                    <th style={{ padding: '12px 16px', fontWeight: 600, textAlign: 'center', borderBottom: '1px solid var(--border-color)' }}>Tổng số nhà cung cấp</th>
                    ...
                    <th style={{ padding: '12px 16px', fontWeight: 600, textAlign: 'right', borderBottom: '1px solid var(--border-color)' }}>Tổng trả</th>
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={8} style={{ textAlign: 'center', padding: '20px' }}>Không có dữ liệu nhập hàng</td>
                    </tr>
                  ) : (
                    data.map((d, i) => (
                      <tr key={i} className={styles.tr}>
                        ...
                        <td style={{ padding: '12px 16px', textAlign: 'center' }}>{d.totalSuppliers}</td>
      ...
      <div className={styles.printArea} style={{ display: 'none' }}>
        <div className={styles.printHeader}>
          ...
          <div className={styles.printHeaderRight}>
            <div className={styles.printTitle}>BÁO CÁO NHẬP HÀNG</div>
          ...
          <tbody>
            {data.map((d, i) => (
              <tr key={i}>
                ...
                <td style={{ textAlign: 'center' }}>{d.totalSuppliers}</td>
