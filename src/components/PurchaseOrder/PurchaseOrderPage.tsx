...
export default function PurchaseOrderPage() {
  ...
  useEffect(() => {
    fetchOrders({ page, pageSize: PAGE_SIZE, keyword, startDate, endDate, supplierId: selectedSupplier?.id ? String(selectedSupplier.id) : undefined, stockId, employeeId, orderType: 'Dathang' });
              ...
              <>
                <table className={styles.table} style={{ minWidth: '1000px' }}>
                  <thead className={styles.thead}>
                    <tr>
                      <th className={styles.thCode}>Mã đặt hàng nhập</th>
                    ...
                    {orders.length === 0 ? (
                      <tr>
                        <td colSpan={9} className={styles.emptyRow}>
                          ...
                          <span>Không có phiếu đặt hàng nhập nào</span>
                        </td>
                      </tr>
                    ) : (
                      orders.map((t) => (
                ...
                <div className={styles.tableFooter}>
                  ...
                  <div className={styles.footerInfo}>
                    Hiển thị {(page - 1) * PAGE_SIZE + (orders.length > 0 ? 1 : 0)} - {Math.min(page * PAGE_SIZE, total)} trên tổng {total} phiếu đặt hàng
                  </div>
                </div>
              </>
            )}
